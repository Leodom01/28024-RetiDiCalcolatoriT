	...
	public static void main(String argv[]) {
		...
		try {
			...
			while((linesToSwap = stdIn.readLine()) != null) {
				bOutStr = new ByteArrayOutputStream();
				dOutStr = new DataOutputStream(bOutStr);
				dOutStr.writeUTF(linesToSwap);
				byte[] portToSend = bOutStr.toByteArray();
				
				packet.setData(portToSend);
				System.out.println("Client: mando "+linesToSwap+" a Discovery");
				socket.send(packet);
				
				packet.setData(data);
				socket.receive(packet);
				System.out.println("pacchetto "+packet.getData());
